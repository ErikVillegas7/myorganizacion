          ...
          {showQuizReq && onUpdateQuizGrade && (
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {Array.from({ length: subject.quizRequiredCount ?? 0 }, (_, i) => (
                <label key={i} className="rounded-2xl border px-3 py-3" style={{ borderColor: "var(--c-border)", background: "var(--c-glass)" }}>
                  <p className="text-xs font-semibold mb-2" style={{ color: "var(--c-text-muted)" }}>Cuestionario {i + 1}</p>
                  <input type="number" min={1} max={10} step={0.5} value={(subject.quizGrades ?? [])[i] ?? ""}
                    onChange={e => onUpdateQuizGrade(i, e.target.value)} placeholder="Nota"
          ...
          {showWorkReq && onUpdateWorkGrade && (
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {Array.from({ length: subject.groupWorkRequiredCount ?? 0 }, (_, i) => (
                <label key={i} className="rounded-2xl border px-3 py-3" style={{ borderColor: "var(--c-border)", background: "var(--c-glass)" }}>
                  <p className="text-xs font-semibold mb-2" style={{ color: "var(--c-text-muted)" }}>Trabajo {i + 1}</p>
                  <input type="number" min={1} max={10} step={0.5} value={(subject.groupWorkGrades ?? [])[i] ?? ""}
                    onChange={e => onUpdateWorkGrade(i, e.target.value)} placeholder="Nota"
                    className="w-full rounded-xl px-3 py-2 text-sm font-medium focus:outline-none"
                    style={{ background: "var(--c-glass)", border: "1px solid var(--c-border)", color: "var(--c-text)" }} />
                </label>
              ))}
